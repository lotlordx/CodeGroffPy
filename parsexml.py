...
def get_movies():
    ...
    dom = get_tree()
    get_all_movies = dom.findall('movie')
    movie_list = []

    for c in get_all_movies:
        title = c.get('title')
...
def get_movie_longest_runtime():
    ...
    dom = get_tree()
    get_all_movies = dom.findall('movie')
    title_with_longet_run_time = ''
    timer = 0

    for c in get_all_movies:
        title = c.get('title')
        running_time = int(c.get('runtime').split()[0])
        if running_time > timer:
            timer = running_time
            title_with_longet_run_time = title

    return title_with_longet_run_time
